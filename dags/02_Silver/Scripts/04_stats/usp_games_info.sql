 ...
 */
CREATE OR REPLACE PROCEDURE silver.usp_update_players_stats() LANGUAGE plpgsql AS $$ BEGIN -- Log start of the procedure
    RAISE NOTICE '=====================================';
...
FROM (
        ...
        SELECT *,
            CAST(
                CASE
                    WHEN game_code BETWEEN 2444470 AND 2444849 THEN '20242025' -- Add new CASE branches here for future seasons as API data expands
                    ELSE '20252026' -- Default for future seasons until updated
                END AS INTEGER
            ) AS season
        FROM bronze.player_stats
    ) bps
    LEFT JOIN silver.player_stats sps ON bps.game_code = sps.game_code
WHERE sps.game_code IS NULL;
-- Ensures only new records are inserted
EXCEPTION
WHEN OTHERS THEN -- Log any errors and re-raise to propagate the exception
RAISE NOTICE '=====================================';
