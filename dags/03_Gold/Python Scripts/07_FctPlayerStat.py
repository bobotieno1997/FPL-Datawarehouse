...
def load_data_to_mysql(df: pd.DataFrame, table_name: str):
    ...
    try:
        ...
        df.to_sql(table_name, con=mysql_engine, if_exists='append', index=False, chunksize=10000, method='multi')
...
def main():
    query = 'SELECT * FROM gold.v_fct_player_stats'
    table_name = 'FctPlayerStats'
